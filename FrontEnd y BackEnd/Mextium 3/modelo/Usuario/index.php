...
$sql = "SELECT p.id, p.nombre, p.precio, p.stock, c.nombre AS categoria, p.estado, p.descripcion, p.imagen
        FROM productos p
...
<head>
    ...
    <style>
body {
    background: linear-gradient(120deg, rgb(79, 22, 211)f 0%, #fffbe7 100%) !important;
...
body::before {
    ...
    background: radial-gradient(circle at 20% 30%,rgba(148, 147, 255, 0.7) 0, #fff0 60%),
...
.main-header {
    ...
    position: relative;
    overflow: visible; /* Cambia de hidden a visible o elimínalo */
}
/* Elimina o ajusta este pseudo-elemento si no lo quieres */
.main-header::after {
...
.logo-tium {
    ...
    margin-left: -5px;
    padding: 0 12px;
    text-shadow: 0 2px 8pxrgba(82, 67, 246, 0.93);
...
.card-producto:hover {
    box-shadow: 0 12px 40px 0rgba(19, 7, 255, 0.33), 0 2px 16px rgba(0,0,0,0.18);
    transform: translateY(-6px) scale(1.03) rotate(-1deg);
    border-color:rgba(7, 36, 255, 0.6);
    z-index: 2;
}
...
.card-producto:hover .card-img-top {
    ...
    box-shadow: 0 8px 32pxrgba(46, 23, 223, 0.33);
}
.badge, .ribbon span {
...
.ribbon span {
    ...
    top: 20px;
    letter-spacing: 1px;
    text-shadow: 0 2px 8px rgba(0, 0, 255, 0.77);
}
...
.promo-banner {
    background: linear-gradient(90deg,rgb(52, 86, 255) 0%,rgb(47, 146, 252) 100%);
    color: #212529;
    border-radius: 12px;
    padding: 1.5rem 1rem;
    margin-bottom: 2rem;
...
.btn-gradient-blue {
    background: linear-gradient(90deg, #007bff 0%,rgb(40, 7, 255) 100%);
    color: #fff;
    border: none;
...
.btn-gradient-blue:hover {
    background: linear-gradient(90deg,rgb(84, 75, 237) 0%,rgb(40, 7, 255) 100%);
    color: #fff;
    box-shadow: 0 4px 16pxrgba(71, 83, 255, 0.33);
}
.footer-mex {
    background: linear-gradient(90deg, #007bff 0%, rgb(40, 7, 255) 100%);
    color: #fff;
    ...
    box-shadow: 0 -2px 16px rgba(48, 142, 243, 0.13);
}
...
@keyframes quoteGlow {
    0% { filter: drop-shadow(0 0 0 rgb(40, 7, 255));}
    100% { filter: drop-shadow(0 0 8px rgb(40, 7, 255));}
}
.gif-hover-zoom {
...
.social-bar a:hover {
    ...
    filter: brightness(1.2) drop-shadow(0 2px 8pxrgba(81, 75, 251, 0.53));
}
...
#sidebar-menu {
    ...
    scrollbar-color:rgb(83, 77, 232) #fffbe7;
}
...
<body>
        ...
        <div class="container-fluid">
            <div class="row align-items-center">
                <div class="col-auto d-flex align-items-center">
                    ...
                    <!-- Logo de Mextium grande y alineado con el encabezado -->
                    <img src="../../img/logo.png" alt="Mextium" style="height:120px;max-width:220px;object-fit:contain;display:block;">
                </div>
                <div class="col d-flex justify-content-center">
                    <!-- Barra de búsqueda centrada y con sombra -->
                    <form class="w-100" style="max-width:600px;" role="search">
                        <div class="input-group input-group-lg shadow rounded-pill bg-white">
                            <input type="search" class="form-control border-0 bg-white rounded-start-pill ps-4" id="busqueda-mextium" placeholder="Buscar productos, tiendas, categorías..." aria-label="Buscar">
    ...
    <div id="sidebar-menu" class="position-fixed top-0 start-0 h-100 bg-white shadow" style="width:320px;z-index:1050;transform:translateX(-100%);transition:transform .3s;">
        ...
        <hr>
<h6 class="fw-bold text-primary mt-4">Carrito de compras</h6>
<div id="carrito-sidebar" class="mb-3"></div>
<hr>
<h6 class="fw-bold text-primary mt-4">Reseñas recientes</h6>
<div id="sidebar-reseñas" class="mb-3"></div>
    </div>
    ...
    <div class="col-lg-3 col-md-4 col-sm-6">
        <div class="card shadow-lg border-0 h-100 card-producto position-relative overflow-hidden glass-card" style="transition:transform .2s;">
            ...
            <div class="card-footer bg-white border-0 text-center">
                <button class="btn btn-gradient-blue w-100" 
                    onclick="verDetalleProducto('<?php echo htmlspecialchars($prod['id']); ?>', '<?php echo htmlspecialchars(addslashes($prod['nombre'])); ?>')">
...
let carrito = JSON.parse(localStorage.getItem('carritoMextium') || '[]');

// Mostrar detalles del producto en modal
function verDetalleProducto(id) {
    document.getElementById('detalleProductoContenido').innerHTML = '<div class="text-center py-5"><div class="spinner-border text-primary"></div></div>';
    var modal = new bootstrap.Modal(document.getElementById('modalDetalleProducto'));
    modal.show();

    fetch('../tienda/detalle.php?id=' + encodeURIComponent(id) + '&ajax=1')
        .then(res => res.text())
        .then(html => {
            document.getElementById('detalleProductoContenido').innerHTML = html;
            document.getElementById('btnAgregarAlCarritoModal').onclick = function() {
...
function agregarAlCarrito(id, nombre) {
    ...
    localStorage.setItem('carritoMextium', JSON.stringify(carrito));
...
function mostrarCarritoSidebar() {
    let contenedor = document.getElementById('carrito-sidebar');
    if (!contenedor) return;
    if (carrito.length === 0) {
        contenedor.innerHTML = '<div class="text-center text-muted py-4">Tu carrito está vacío</div>';
        return;
    }
    ...
    carrito.forEach((item, idx) => {
        html += `<li class="list-group-item d-flex align-items-center justify-content-between">
            <div>
                <span class="fw-bold text-primary me-2">${item.nombre}</span>
                <button class="btn btn-sm btn-outline-secondary ms-1" onclick="editarCantidadCarrito(${idx})" title="Editar cantidad">
                    <i class="fa fa-edit"></i>
                </button>
                <button class="btn btn-sm btn-outline-danger ms-1" onclick="borrarDelCarrito(${idx})" title="Eliminar">
                    <i class="fa fa-trash"></i>
                </button>
            </div>
            <div class="d-flex align-items-center">
                <span class="badge bg-gradient bg-warning text-dark fs-6 px-3 py-2 me-1" style="font-size:1.1rem;min-width:38px;">
                    <i class="fa fa-cubes me-1"></i> ${item.cantidad}
                </span>
            </div>
        </li>`;
    });
    html += '</ul><button class="btn btn-success w-100">Finalizar compra</button>';
...
function editarCantidadCarrito(idx) {
    let nuevaCantidad = prompt('Nueva cantidad:', carrito[idx].cantidad);
...
// Ejemplo de reseñas (puedes reemplazar por fetch/AJAX si tienes backend)
const reseñasEjemplo = [
    {
        usuario: "Ana G.",
        texto: "¡Excelente producto y entrega rápida!",
        estrellas: 5
    },
    {
        usuario: "Luis P.",
        texto: "Todo llegó en buen estado, recomendado.",
        estrellas: 4
    },
    {
        usuario: "Sofía R.",
        texto: "Muy buena atención y variedad.",
        estrellas: 5
    }
];

// Mostrar reseñas en el sidebar
function mostrarReseñasSidebar() {
    const contenedor = document.getElementById('sidebar-reseñas');
    if (!contenedor) return;
    let html = '<ul class="list-group">';
    reseñasEjemplo.forEach(r => {
        html += `<li class="list-group-item small">
            <div class="fw-bold text-primary">${r.usuario}</div>
            <div class="text-warning mb-1">
                ${'★'.repeat(r.estrellas)}${'☆'.repeat(5 - r.estrellas)}
            </div>
            <div class="text-muted">${r.texto}</div>
        </li>`;
    });
    html += '</ul>';
    contenedor.innerHTML = html;
}
mostrarReseñasSidebar();
    </script>
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-gradient-blue" id="btnAgregarAlCarritoModal">
